...
func (pb *OWServerPB) updateSensor(nodeHWID string, sensorNode *XMLNode) {
	...
	// 		// 	types.DefaultInputInstance, app.HandleAction)
	// 	}
	// }
...
func (pb *OWServerPB) updateDevice(deviceOWNode *XMLNode) {
	props, _ := pb.edsAPI.ParseNodeParams(deviceOWNode)
	...
	// device := pb.GetNodeByHWID(nodeHWID)
	// if device == nil {
	...
	for _, propXML := range deviceOWNode.Nodes {
		...
		pb.updateSensor(nodeHWID, &propXML)
	}
}
...
func (app *OWServerPB) updateGateway(gwParams map[string]string) {
